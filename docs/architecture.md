...
The PM dashboard sweep (`/api/cron/pm-dashboard`) takes the shared company worker lease like every other background Procore reader, re-polls each active project every `PM_DASHBOARD_REPOLL_MINUTES` (default 90) now that RFI/Task Item/Meeting changes arrive via project webhooks, and its background wrapper stops iterating when the lease is busy or Procore is cooling down.
...
`POST /api/webhooks/procore/process` requires the sync secret, claims due queue entries, dispatches resource-specific handlers, updates canonical and compatibility tables, retries transient failures with backoff, and can enqueue project onboarding work. Potential Change Order and Change Order Package handlers fetch the current Procore record and immediately enqueue commitment verification when its authoritative status is Approved. RFI, Task Item, and Meeting events call `syncPmDashboardActionItem`, which fetches the single record through the centralized Procore client and upserts it into `pmc_action_items` (a 404 removes the mirror row; other errors propagate so the queue retries). `/api/cron/change-order-approvals` and `/api/cron/pm-dashboard` remain as reconciliation sweeps for projects whose hooks are not yet registered. This separation keeps webhook acknowledgement fast and processing durable.

Procore exposes two webhook catalogs. The company-level catalog only contains company/portfolio resources (Projects, Company Users, Project Stages, ...). Project-tool resources — RFIs, Task Items, Meetings, Potential Change Orders, Change Order Packages, Timecard Entries, Productivity Logs — are only available on a per-project hook at `/rest/v2.0/companies/{companyId}/projects/{projectId}/webhooks/hooks`. The shared trigger plan and catalog resolver live in `src/lib/procoreWebhookPlan.js`; project triggers are grouped (`priority` = PM dashboard + change orders, `actuals` = timecards + productivity) so higher-volume resources can be enabled separately.

Registration paths:
- `scripts/registerProcoreWebhook.mjs --register` maintains the company hook.
- `scripts/registerProcoreWebhook.mjs --register-projects [--groups priority] [--limit N] [--dry-run]` walks active `pmc_projects`, ensuring one namespace hook per project with the requested trigger groups. It is idempotent, paces requests (default 1.1 s), waits for the Procore window reset on 429, and can be re-run to resume.
- `/api/cron/project-onboarding` calls `ensureProjectWebhookHook` (`src/lib/procoreProjectWebhooks.ts`) for every newly onboarded project so hooks stay current without manual runs. The step is recorded in the onboarding log but never blocks the rest of onboarding.
...
Commitment Maker defaults untyped lines to the Materials (`M`) cost type. For approved change orders, the source line's authoritative project WBS ID remains valid for commitment field tracking even when that WBS code has no project budget line; this fallback does not create or alter a budget line.
...
The five-minute Netlify scheduler dispatches `pm-dashboard-sync-background`, which rotates through active canonical projects via `pmc_action_item_sync_state`. `/api/cron/pm-dashboard` uses the service-account lane and centralized Procore client to fetch RFIs, Task Items, and Meetings, then idempotently upserts each successful source snapshot. An unavailable or failed Procore tool preserves its previous rows; a successful empty snapshot clears stale rows for that project and source. When the RFI, Task Item, and Meeting webhook triggers are registered, individual changes also arrive through the webhook queue and are applied per record, so the polling sweep acts as the reconciliation safety net rather than the primary freshness path. Page renders and `/api/pm-dashboard` never call Procore directly.
