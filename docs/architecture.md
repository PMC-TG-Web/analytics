...
The background wrappers call `/api/cron/actuals`, `/api/cron/nightly-structure`, `/api/cron/project-onboarding`, and `/api/cron/project-reconciliation` in bounded batches. Queue state, locks, retry timestamps, and Procore quota state live in PostgreSQL so work can resume across invocations. The shared Procore client records low-water quota headers, honors the provider reset epoch, and reserves a configurable request balance for interactive and mutation traffic. Every sync-secret request checks that shared cooldown before calling Procore; user-session requests remain available. Provider-wide throttling defers claimed queue records until the reset with deterministic jitter and does not increment project failure counters. Health stays quiet for backpressure inside the data SLA, identifies quota recovery when data becomes stale, and reserves repeated-project alerts for project-specific failures. The full active-project reconciliation runs daily at 07:10 UTC and health monitoring allows 26 hours between successful completions. The Actuals wrapper advances due Actuals projects before secondary estimate, onboarding, and purchase-order work can consume the shared Procore quota. Successful daily structure and estimate records requeue five minutes short of 24 hours so second-level scheduler jitter cannot push them past the final nightly tick. Secret-authenticated operators can pass one exact `projectId` to `/api/cron/nightly-structure` for a targeted structure rerun.
