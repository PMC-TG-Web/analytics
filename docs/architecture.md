...
The background wrappers call `/api/cron/actuals`, `/api/cron/nightly-structure`, `/api/cron/project-onboarding`, and `/api/cron/project-reconciliation` in bounded batches. Queue state, locks, retry timestamps, and rate-limit cooldowns live in PostgreSQL so work can resume across invocations. The full active-project reconciliation runs daily at 07:10 UTC and health monitoring allows 26 hours between successful completions. In the Actuals wrapper, a shared Procore rate limit stops secondary estimate, onboarding, and purchase-order work for that invocation but still advances into the Actuals loop so secondary work cannot starve due Actuals projects. Successful daily structure and estimate records requeue five minutes short of 24 hours so second-level scheduler jitter cannot push them past the final nightly tick. Secret-authenticated operators can pass one exact `projectId` to `/api/cron/nightly-structure` for a targeted structure rerun.
