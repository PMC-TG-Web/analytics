...
The shared Procore client records low-water quota headers, honors the provider reset epoch, and reserves a configurable request balance for interactive and mutation traffic. Secret-authenticated route responses include the exact observed cooldown and the number of outbound Procore requests, allowing parent workers to retain provider reset timing and record per-step request consumption. Every sync-secret request checks that shared cooldown before calling Procore; user-session requests remain available. Provider-wide throttling defers claimed queue records until the exact reset with deterministic jitter, is logged as a successful deferral rather than a project failure, and does not increment project failure counters.
