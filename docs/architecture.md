...
### Entered monthly Food total

`FoodTotalInput.tsx` saves a nonnegative dollar total through the authenticated same-origin `POST /api/accounting/direct-cost-bills/food-total` endpoint. `QboBillFoodTotal` is keyed by company + explicit project ID + month. Every save uses an optimistic revision and transactionally appends a `QboBillFoodTotalRevision` audit with the session operator. Shared storage makes totals available on every machine. Saving a total never posts QBO.

`loadQboDirectCosts.ts` omits Food/Food Cost productivity entries from quantity/rate calculations and catalog matching. When Food appears in monthly logs, an unset total blocks posting until a dollar total or explicit $0 is saved. A positive total produces exactly one `manual_food` line with stable key `food:01-300-10-80`, quantity 1, unit `total`, and unit cost/amount equal to the entered dollars. No PO price or log quantity is multiplied. Zero omits Food. The queue includes projects with saved Food totals even without logs. POST always rebuilds this draft from PostgreSQL.

The QBO host's `manual-food-bill-line.js` validates project/month attribution, saved revision, fixed code/material type, quantity 1 and `.M` product mapping. Food uses the normal project customer, Flatwork class, Direct Costs - offset and monthly replacement ledger. Other lines still require current catalog evidence. Food-only drafts do not depend on a catalog price. Totals are replaced on update, never added again. The existing all-lines-empty bill restriction remains.

Migration `20260922140000_qbo_bill_food_totals` adds only the total and audit tables. Validation: `node --test test/qboFoodTotal.test.mjs test/qboFoodTotalLoader.test.mjs test/qboFoodTotalRoute.test.mjs test/permissions.test.mjs`; QBO_1: `node --test test/manual-food-bill-line.test.js test/direct-cost-bill.test.js test/direct-cost-project-setup.test.js test/monthly-bill-sync.test.js`.
