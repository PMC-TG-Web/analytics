...
Cost Code P&L loads selected estimate lines and payload aliases through `src/lib/loadCostCodeSalesLines.ts` in 1,000-row ID pages, bounding database gateway responses as history grows. Company/proposal filters and explicit line identities are preserved. Validate with `node --test test/loadCostCodeSalesLines.test.mjs test/costCodeSalesAnalytics.test.mjs`.
...
Direct-cost pricing uses `qboCostCatalog.ts` for normalization, positive prices, compatible units, unique item matching, and freshness validation. `loadQboCostCatalog.ts` reads the complete company snapshot from `ProcoreSyncProjectState.lastResult` under project `__company__`, dataset `qbo_cost_catalog`. `qboCostCatalogSync.ts` uses the central Procore client and existing shared worker lease to read every catalog page and publish atomically; partial failures preserve prior prices. Catalog checks are shared across tabs/machines with a five-minute cooldown. Snapshots older than 24 hours block posting. `loadQboDirectCostLaborRates.ts` now uses hourly catalog LABOR items, with no estimate queries or bid-board linkage. Draft v3 carries catalog evidence; the compatible QBO_1 builder validates it before saving. Ordinary GETs remain database-only. Validate with `node --test test/qboCostCatalog.test.mjs test/qboCostCatalogSync.test.mjs test/qboDirectCostLaborRates.test.mjs test/qboDirectCostLabor.test.mjs` and QBO_1's direct-cost-bill tests.

While the direct-cost bill page is visible, it polls the accounting-protected, same-origin POST `/api/accounting/direct-cost-bills/sync`. This controlled ingestion path reuses the existing PO detail sync and server-only sync-secret bypass. Each request first refreshes the company catalog if due; otherwise it checks at most one project with productivity logs in the selected month, in oldest-attempt order. The shared Procore worker lease/cooldown prevents overlapping background workers; `ProcoreSyncProjectState` dataset `bill_review_po` prevents rechecking a project within five minutes across tabs/machines. Failures retain saved data and schedule another attempt. The page pauses checks while hidden or posting, refreshes the database-backed queue, and reloads the expanded review after a catalog refresh or that project's PO check. Ordinary GETs remain database-only; automatic refresh never posts QBO bills. Daily logs retain their existing ingestion schedules. Validate with `node --test test/qboBillSourceRefresh.test.mjs test/permissions.test.mjs` and `npx tsc --noEmit`.
...
Direct-cost catalog mapping is editable within each expanded project review using `CatalogMappingPanel.tsx` and `/api/accounting/direct-cost-bills/catalog-mapping`. The existing accounting-direct-cost-bills permission, signed-in actor, same-origin POST validation, and configured-company boundary protect this surface. GET lists current compatible-unit, positive-price catalog items from PostgreSQL. POST accepts a catalog item ID, never a rate, and saves `QboCostCatalogMapping` by company + explicit Procore project + PO line ID. An operator may explicitly select an item under a different catalog cost code; the source/QBO product code stays unchanged. A normalized source signature blocks stale mappings after meaningful PO identity/unit/code changes; a revision check prevents concurrent saves overwriting each other. Clearing a choice returns to automatic matching without deleting history metadata. The selected catalog item's current price is used on every read; missing/invalid catalog data blocks posting. Catalog choices and QBO product setup never post a bill. Validate with `node --test test/qboCatalogMapping.test.mjs test/qboCatalogMappingRoute.test.mjs test/permissions.test.mjs`, `npx tsc --noEmit`, and the production build. Migration: `20260921140000_qbo_cost_catalog_mappings`.

Completed primary-estimate imports are released for replacement only after a successful live commitment listing omits every recorded PO and each exact PO returns 404 NOT_FOUND. Partial/uncertain and legacy imports remain blocked. A conditional transaction marks the claim deleted and archives the target IDs in an audit; creation rechecks absence and atomically replaces the claim with empty targets and the new fingerprint. Restored POs and failed reads block recreation. Validate with test/commitmentMakerDeletedEstimate.test.mjs and the opt-in rolled-back test/commitmentMakerPrimaryEstimate.database.test.mjs.

Commitment Maker expands generic hourly Labor labels (including numbered labels such as 1. Labor) to the company catalog scope for exact known labor cost codes before consolidation. Specific descriptions such as Travel Labor remain unchanged; unknown codes are not guessed. This shared transformation covers primary-estimate/workbook parsing and combined groups. Quantities and pricing are preserved. Validate with test/commitmentMaker.test.mjs and test/commitmentMakerPrimaryEstimate.test.mjs.
...
Food / Food Cost PO lines (including numbered CO prefixes) use a bill-only coding override in `qboDirectCostCoding.ts`: cost code `01-300-10-80`, Materials, producing the configured `.M` QBO product under the project prefix and Direct Costs - offsets. The monthly draft and catalog picker use the same coding. Source records, original descriptions, units, quantities and prices are not rewritten. Labor is not reclassified. Food pricing is now supplied by the entered monthly total described below; other items retain current catalog pricing validation. Validate with `node --test test/qboDirectCostCoding.test.mjs test/qboCatalogMapping.test.mjs`.


### Monthly Food expense ledger

`FoodTotalInput.tsx` adds dated expenses with an optional note and shows entries plus running totals for the selected project/month. `POST /api/accounting/direct-cost-bills/food-total` now requires `operation: add` and a client-generated entry UUID. The existing session, same-origin, company and project checks apply; older total-replacement requests are rejected. Each immutable `QboBillFoodEntry` stores date, amount, note, operator and timestamp. In one transaction, an atomic increment updates `QboBillFoodTotal` and appends its revision audit. Concurrent additions preserve both amounts. Repeated entry IDs with identical details return the previous success; changed details cannot reuse an ID. The browser retains pending requests in session storage so timeout/reload retries cannot double-add an expense.

`loadQboDirectCosts.ts` reads the ledger and total in a repeatable-read snapshot and blocks posting if their amounts disagree. Food/Food Cost productivity entries are excluded from quantity/rate calculations and catalog mapping. Food entries are optional even when daily logs contain Food. An unset ledger omits Food without blocking bill creation or updates; expenses can be added later. A positive accumulated balance produces one `manual_food` line (`food:01-300-10-80`), quantity 1, unit `total`, with unit cost/amount equal to the ledger total. Zero omits Food. The monthly queue includes projects with saved Food balances even without logs. All browser machines share the same ledger.
...
Catalog matching accepts a unique normalized catalog description as well as item name, within the same cost code and labor/material kind. Unit, positive-price and ambiguity checks still apply. Concrete material code 05-100-10-20 (set/fill bollards) is excluded before pricing and mapping; steel bollards at 05-100-10-10 and labor remain included. Validate with node --test test/qboCostCatalog.test.mjs test/qboCatalogMapping.test.mjs test/qboDirectCosts.test.mjs.

Commitment Maker exposes Reset to Original Groups outside the multi-PO combine panel so it remains available after combining every group into one. Primary-estimate preview uses resetEstimateGrouping to clear saved combinations transactionally only for a deleted import (or no saved import). Running, uncertain, retryable and completed imports cannot discard their grouping. The reset audits previous combinations, preserves targets/fingerprint/history, and re-previews the original source groups; it never mutates Procore POs. Workbook reset remains local. Validate test/commitmentMakerGroupingReset.test.mjs and the opt-in rolled-back test/commitmentMakerPrimaryEstimate.database.test.mjs.

Direct-cost matching recognizes full rebar size/length shorthand such as #4x20' rebar. A unique exact catalog name under another code reports the candidate and both codes for operator confirmation. Known concrete purchase descriptions are excluded despite legacy PO codes; hourly Labor stays included, while exact concrete descriptions mislabeled Labor and measured in cubic yards are omitted. Setup links directly to the detailed issue list. Validation: node --test test/qboCostCatalog.test.mjs test/qboCatalogMapping.test.mjs test/qboDirectCosts.test.mjs.

Legacy base-estimate duplicate checks exclude audits explicitly sourced from the Approved Change Order sheet or approved_change_order source type, as well as modern sourceChangeOrder metadata. Older CO audits did not store sourceChangeOrder; their absence alone is not evidence of a base estimate. Titles are not used to classify the source. Validate test/commitmentMakerDeletedEstimate.test.mjs.

Primary-estimate creation yields between confirmed Procore operations after a fifteen-second mutation batch. Each accepted line and completed PO result is checkpointed in the existing targets JSON; resumable claims retain the exact source fingerprint and target IDs. Completed POs are skipped after live identity/vendor/status checks. A saved retryable claim returns 202 continuing, and the browser keeps its loading state while issuing bounded continuations with the original prepared source ID. Unknown mutation outcomes never continue automatically. No schema migration is required. Validate test/commitmentMakerCreationBatch.test.mjs, test/commitmentMakerRequest.test.mjs and the opt-in rolled-back estimate claim database test.

QBO setup automatically refreshes an existing source-line product assignment when only its cost code changes within the same project prefix and material/labor suffix, with unchanged unit and class. It reuses/creates the target product without renaming the old QBO product, preserves accounting configuration, and audits previous and new mappings. Shared service logic supports a future nightly worker; scheduling and automatic bill posting are not enabled by this change. Setup options show the previous assignment. Validate in QBO_1: node --test test/direct-cost-project-setup.test.js test/direct-cost-bill.test.js test/direct-cost-bill-service.test.js.

Exact normalized catalog names/descriptions can supply prices across cost codes when units and labor/nonlabor kind agree and all matching alternatives have the same positive rate. The source/QBO cost code is retained; explicit catalog IDs and saved choices retain their existing checks. Matching ignores known placement suffixes including SOG, Site, walls, foundations and slab-on-deck labels. Approved per-each Somero Power Rake and S-840 eight-hour-minimum charges use Materials/Direct Costs - even if the PO says Labor; hourly operator labor remains unchanged. Validate with test/qboCostCatalog.test.mjs, test/qboDirectCostCoding.test.mjs and test/qboCatalogMapping.test.mjs.

### Preserve QBO-added bill lines (2026-09-23)

Direct Cost Bills reconciliation offers `preserveAdditions`: the host identifies QBO-only expense lines using saved managed line IDs, or exact sent-field matching against successful historical posting evidence for older bills. Confirmation records the reviewed additions and audit in the monthly receipt. Monthly updates replace Procore lines and offsets, retain the added lines with their QBO IDs, and keep source-only fingerprints for idempotency. Successful create/update receipts capture managed QBO line snapshots. Missing legacy evidence or ambiguous edited/deleted legacy source lines block preservation instead of guessing. Replacement remains available explicitly. No bill is written during confirmation. Tests: host `bill-reconciliation.test.js`, `monthly-bill-sync.test.js`, and app `qboBillReconciliationRoute.test.mjs`, `qboBillReconciliationPanel.test.mjs`.

Screeding equipment bill classes (2026-09-23): host `screeding-classes.js` applies company-scoped `setup-defaults.json.screeding` to equipment/.E items and approved legacy Somero .M lines. Power Rake, S15/S15R, SRS/SRS4, and S840/S940 resolve to their configured Screeding subclasses; S840 uses Screed-940. The 3D Profiler, other equipment and labor retain Flatwork. No product is renamed. Offsets group by category and resolved class, with no project on offsets. Preview returns itemClasses and offsetLines from the actual prepared payload; fingerprints include the class changes. Unknown or ambiguous screeders require review. Validate host test/screeding-classes.test.js, test/direct-cost-bill.test.js and test/direct-cost-bill-service.test.js.

Screeding product suffix (2026-09-23): `screeding.equipmentProducts` makes setup select `.E` for the approved screeder identities, including legacy Somero `.M` sources. Setup can refresh an existing same-code `.M` assignment to `.E` while preserving its unit, accounting template, offsets and audit history; exact existing equipment products are reused. The original material product is not renamed. Bill preparation blocks stale `.M` screeder assignments until setup refreshes them. The 3D Profiler remains `.M`. Current September assignments were refreshed for Evergreen, Pro Ag and Sadsbury; this changed products/mappings only, with no bill posting. Tests: QBO_1 test/direct-cost-project-setup.test.js, test/screeding-classes.test.js, test/direct-cost-bill.test.js.

Project line rules (2026-09-23): `QboBillLineRule` is keyed by company + project + source line key; purchase lines use the Procore PO line ID and labor uses labor:costCode. Ignore rules persist across months until restored and remove matching source logs/timecards before aggregation, pricing checks and offset calculation. Optional positive purchase unit prices override catalog prices for that project/source only, with source-signature validation. `QboBillLineRuleRevision` records every setting atomically with optimistic revision checks. POST /api/accounting/direct-cost-bills/line-rule requires the existing direct-cost-bills permission, session and same-origin CSRF checks. ProjectLineRules offers ignore/restore and project price inputs; Food remains managed by its ledger. Project-price evidence is included in the draft and validated by the QBO host against company/project/source identity before posting. Migration 20260923160000_qbo_bill_line_rules was applied to the configured shared application database. Pro Ag source 598134418331953 has the explicitly authorized $1,520 project price. Tests: qboBillLineRules.test.mjs, qboFoodTotalLoader.test.mjs, permissions.test.mjs; host direct-cost-bill.test.js.

### Existing QBO screeder prices (2026-09-23)

The shared QBO host can enable `setup-defaults.json.screeding.qboManagedPrices`. First creation uses the source draft price. Later project reviews and posting read the exact saved QBO bill and retain the unit price and description of each owned screeder line. Procore supplies month-to-date quantities; the host recalculates its matching class offset. Added QBO expense lines are retained by bill-line ID. Receipts store source-line-to-QBO-line bindings, managed line snapshots, and saved QBO prices. Queue reads use the saved prices without calling QBO. Preview totals use trusted host prices; POST rebuilds the database draft and rechecks QBO's version. Other managed-line changes still need review. Legacy ownership must be unambiguous; Pro Ag's September bill received a separately audited ownership adoption without writing to QBO.

Host validation: `node --test test/qbo-managed-bill.test.js test/direct-cost-bill-service.test.js test/monthly-bill-sync.test.js test/bill-reconciliation.test.js`. The host implementation remains in the separate `QBO_1` workspace.

Legacy QBO screeder ownership correction (2026-09-23): `QBO_1/src/qbo-managed-bill.js` resolves saved description plus structural fields before permitting a unique renamed-screeder fallback. Price and amount are excluded from screeder identity; shared product IDs and quantities alone must not collapse distinct machines. Named lines are resolved first regardless of row ordering. Sadsbury's existing Power Rake/S-840 lines now bind automatically without a receipt repair or QBO write. Regression coverage: `test/qbo-managed-bill.test.js`, including shared products, changed prices, reordered lines, a renamed machine, and ambiguous additions.

Direct-cost unit mismatch diagnostics (2026-09-23): aggregate purchase lines retain PO number/title for host validation messages. Unit mismatches identify the item description, cost code, Procore unit, saved QBO product and unit, PO, and affected daily-log dates. This metadata does not change quantities, prices, product assignments, or posting fingerprints. Tests: Analytics `test/qboDirectCosts.test.mjs`; QBO host `test/direct-cost-bill.test.js`.

### Monthly QBO item aggregation (2026-09-23)

The shared host applies `aggregate-bill-items.js` after source validation and retained QBO prices, before review, reconciliation, or posting. Lines with the same product, normalized description, unit, rate, class/customer, billable status and offset destination combine across PO source IDs. Quantities use exact decimal sums, amounts round once per output line, and any rounding delta adjusts the corresponding category/class offset. Manual QBO additions are appended afterward and stay untouched. `source[]` retains original evidence; `itemSourceKeys` records all original source keys for each output item. Receipts bind multiple source keys to the same QBO line ID, with unique ID retention when groups later split. Original source drafts remain separate for validation and setup. The host returns `itemGroups` for the item-details preview; the UI provides expandable source PO/date/quantity details. Saving an existing bill replaces its prior rows rather than appending another aggregate.

Validation: host `test/aggregate-bill-items.test.js`, `test/qbo-managed-bill.test.js`, `test/direct-cost-bill-service.test.js`; app `test/qboBillDisplayLines.test.mjs`, TypeScript and focused lint. The main app does not post a bill during rollout.

Project Management hours-only billing rule (2026-09-24): cost code 01-300-10-20 is excluded from QBO direct-cost bill PO usage and timecard aggregation before pricing validation or labor-rate fallback. Procore source records and analytics hours are unchanged. Existing managed bill lines are removed only on the next bill update; manual QBO additions retain their existing preservation behavior. Validation: node --test test/qboHoursOnlyCosts.test.mjs test/qboDirectCosts.test.mjs test/qboFoodTotalLoader.test.mjs.

PO price fallback (2026-09-24): unmatched catalog descriptions use a positive synchronized PO unit price when the current catalog contains the source cost code. Explicit saved/catalog identities and named catalog matches retain their validation; unknown codes and nonpositive PO prices remain blocked. The PO unit is preserved. PurchasePriceEvidence records company, project, source line, unit, price and catalog code identity; the QBO host validates and retains it in the posting fingerprint. Preview labels the PO fallback separately from catalog pricing. Tests: qboCatalogMapping.test.mjs and host direct-cost-bill.test.js.

### Direct-cost bill budget prerequisites (2026-09-24)

Setup and bill POST call ensureQboBudgetReadiness before product creation or bill posting. Required budget codes come from the host budget-plan operation during setup and the freshly prepared actual product mappings when posting, including equipment .E suffixes. Ordinary GET/queue reads remain database-only for Procore. One code is prepared per HTTP 202 continuation under the existing company worker lease; only missing active WBS combinations and zero-original-budget rows are created. Existing budgets are never edited. Project/standard segment identities are exact; unsupported custom WBS layouts and missing company codes are explicit errors. The ProcoreSyncProjectState dataset qbo_bill_budget_setup uses lastError as a durable JSON pending-write intent (code/kind). A lost mutation response must be resolved by a live read; an absent uncertain write is never replayed. Explicit rejection clears the intent; 429 yields an automatic continuation. This is budget readiness, not proof of the connector's internal ERP mapping or completed job-cost import. Connector job-cost sync remains separate. Tests: qboBudgetReadiness.test.mjs and host direct-cost-project-setup.test.js.

Budget readiness reuses exact code/type evidence verified within 24 hours, scoped to company and project, from complete live budget snapshots in qbo_bill_budget_setup.lastResult or synchronized budgetLineItem rows. An outstanding mutation intent disables this shortcut. Missing, duplicate, expired or invalid evidence still requires live validation; changes to required product codes cannot reuse unrelated evidence. This avoids consuming Procore quota on every setup continuation and bill save. Repair snapshots retain their original verification timestamps.
...
Nightly structure and its four authenticated sync stages use `procoreSyncStream.ts` whitespace heartbeats to avoid idle-proxy timeouts. Only authorized worker requests stream. Final JSON carries `httpStatus`, explicit failure/rate-limit flags, quota deadline and request counts because streaming commits HTTP 200 early. Interactive stage responses retain their original HTTP contract. Test: `node --test test/procoreSyncStream.test.mjs`.

Direct-cost UOM policy (2026-09-25): catalog and saved QBO product UOM differences do not block pricing/setup/posting. Use source quantity times selected current unit cost with no conversion; retain source and catalog UOM as evidence. Positive prices, item identity, dimensions, cost-type and ambiguity validations remain. Fully dimensioned sonotube names match Standard Wall Construction of the same diameter and length. Host validations updated in direct-cost-bill.js and direct-cost-project-setup.js.
