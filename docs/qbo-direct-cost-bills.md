...
The first version includes approved, non-deleted productivity logs, excluding
billing-file records and zero usage. It sums `quantityUsed`, never
`previouslyUsed` or delivered quantities. Company + Procore project + explicit
line-item ID (including established aliases) determine identity. Unit costs
come from the current mirrored PO line, not a reconstructed historical rate.
Missing/ambiguous lines, invalid quantities, missing units, and nonpositive
rates block export. Version 2 includes timecard labor grouped by exact cost code. Hourly costs come
from `cost_item.unit_labor_cost` in the selected primary/base estimate, using
the existing cost-item-ID crosswalk. PO lump-sum/equipment rates are not hourly
labor rates. Timecards without a source approval status are included; missing,
negative, or duplicate source hours block export. Missing/conflicting rates
remain visible as unpriced hours and block the whole bill. Labor present in
both productivity and timecards is flagged for source selection. Negative
category offsets are generated by the QBO integration from its saved mapping.
