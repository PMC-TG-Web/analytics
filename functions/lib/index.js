...
exports.aggregateProjectData = void 0;
 ...
 */
exports.aggregateProjectData = functions.firestore
    ...
    await db.runTransaction(async (transaction) => {
        ...
        if (!summary.contractors)
            summary.contractors = {};
        if (!summary.pmcGroupHours)
            summary.pmcGroupHours = {};
        // Helper to update metrics
        const updateMetrics = (projData, multiplier) => {
            if (!projData)
                return;
            ...
            const isExcluded = projData.projectArchived === true ||
        ...
        for (const key in summary.contractors) {
            if (summary.contractors[key].count <= 0)
                delete summary.contractors[key];
        }
