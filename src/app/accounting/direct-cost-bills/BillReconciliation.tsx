...
type Review = { fingerprint: string; billNumber: string; current: { rows: Row[]; total: number; note: string }; proposed: { rows: Row[]; total: number; note: string } };
...
export default function BillReconciliation({ companyId, projectId, month, disabled, onBusy, onComplete }: { companyId: string; projectId: string; month: string; disabled: boolean; onBusy: (busy: boolean) => void; onComplete: () => Promise<void> }) {
  ...
  if (!opened) return <button disabled={disabled} className="rounded border border-blue-700 px-4 py-2 text-blue-800 disabled:opacity-50" onClick={() => request('preview')}>Reconcile QBO changes</button>;
  return <section className="space-y-4 rounded-xl border border-amber-300 bg-amber-50 p-5" aria-label="Reconcile QBO changes">
    <h3 className="font-semibold">Reconcile QBO changes{review ? ` · ${review.billNumber}` : ''}</h3>
    <p className="text-sm">Review the current QBO bill and the proposed monthly replacement. Confirming saves the reviewed version and an audit copy. Then use Update bill in QBO to replace its item lines and negative offsets. Manual line changes will be replaced.</p>
    ...
    {review && <>{([['Current QBO bill', review.current], ['Proposed monthly bill', review.proposed]] as const).map(([title, bill]) => <div key={title} className="overflow-x-auto rounded border bg-white"><h4 className="p-3 font-medium">{title}</h4><table className="w-full text-left text-sm"><thead><tr>{['Line / product', 'Class / project', 'Quantity', 'Cost per', 'Amount'].map(label => <th key={label} className="p-3">{label}</th>)}</tr></thead><tbody>{bill.rows.map((row, i) => <tr key={i} className="border-t"><td className="p-3">{row.description}<div className="text-xs text-slate-500">{row.type} · {row.product}</div></td><td className="p-3">{row.className || '—'}<div className="text-xs">{row.customer || 'No project'}</div></td><td className="p-3">{row.quantity ?? '—'}</td><td className="p-3">{row.unitCost === null ? '—' : money(row.unitCost)}</td><td className="p-3">{money(row.amount)}</td></tr>)}</tbody></table><p className="border-t p-3 font-medium">Net total: {money(bill.total)}</p>{bill.note && <p className="whitespace-pre-wrap px-3 pb-3 text-sm">Note: {bill.note}</p>}</div>)}
      <label className="flex items-start gap-2 text-sm"><input type="checkbox" disabled={working} checked={accepted} onChange={e => setAccepted(e.target.checked)} />I reviewed the changes and want the next update to replace the QBO bill with these monthly costs.</label>
    </>}
    <div className="flex gap-3"><button disabled={working} className="rounded border px-4 py-2 disabled:opacity-50" onClick={() => { setOpened(false); setReview(null); onBusy(false); }}>Cancel</button>{review ? <button disabled={working || !accepted} className="rounded bg-blue-700 px-4 py-2 text-white disabled:opacity-50" onClick={() => request('confirm')}>Confirm reconciliation</button> : <button disabled={working} className="rounded border px-4 py-2 disabled:opacity-50" onClick={() => request('preview')}>Reload comparison</button>}</div>
  </section>;
}
