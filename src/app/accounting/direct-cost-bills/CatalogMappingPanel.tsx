...
type Item = { lineItemId: string; description: string; costCode: string | null; uom: string | null; issue: string | null; catalogName: string | null; manual: boolean; projectPrice?: boolean; poPrice?: boolean };
export default function CatalogMappingPanel({ companyId, projectId, items, disabled, onBusy, onComplete }: { companyId: string; projectId: string; items: Item[]; disabled: boolean; onBusy: (busy: boolean) => void; onComplete: () => Promise<void> }) {
  ...
  return <section id="bill-catalog-mappings" className="scroll-mt-20 rounded-xl border border-blue-200 bg-white p-5 space-y-3">
    ...
    {(showAll ? items : unresolved).map(item => <div key={item.lineItemId} className="rounded-lg border border-slate-200 p-3 space-y-3">
      <div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-sm font-medium">{item.description}</p><p className="text-xs text-slate-500">{item.costCode} · {item.uom || 'Unit missing'}{item.poPrice ? ' · PO price (catalog code matched)' : item.projectPrice ? ' · Project-specific price' : item.catalogName ? ` · ${item.manual ? 'Saved' : 'Automatic'} match: ${item.catalogName}` : ''}</p></div><button disabled={disabled || busy || (!!editing && editing !== item.lineItemId)} onClick={() => open(item.lineItemId)} className="rounded border border-blue-300 px-3 py-1 text-sm text-blue-800 disabled:opacity-50">{item.issue ? 'Confirm pricing source' : 'Change pricing source'}</button></div>
      ...
      {editing === item.lineItemId && <div className="space-y-3 border-t pt-3">
        ...
        {choices && <>
          ...
          <p className="text-xs text-slate-500">Catalog items with a positive current price are listed. Quantities multiply by the selected unit cost; units are not converted.</p>
