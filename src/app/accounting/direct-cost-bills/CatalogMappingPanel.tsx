...
export default function CatalogMappingPanel({ companyId, projectId, items, disabled, onBusy, onComplete }: { companyId: string; projectId: string; items: Item[]; disabled: boolean; onBusy: (busy: boolean) => void; onComplete: () => Promise<void> }) {
  ...
  async function save(clear = false) {
    ...
    } catch (e) { setError(e instanceof Error ? e.message : 'Unable to save mapping.'); }
    finally { setBusy(false); }
  }
  ...
  return <section className="rounded-xl border border-blue-200 bg-white p-5 space-y-3">
    ...
    {(showAll ? items : unresolved).map(item => <div key={item.lineItemId} className="rounded-lg border border-slate-200 p-3 space-y-3">
      <div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-sm font-medium">{item.description}</p><p className="text-xs text-slate-500">{item.costCode} · {item.uom || 'Unit missing'}{item.catalogName ? ` · ${item.manual ? 'Saved' : 'Automatic'} match: ${item.catalogName}` : ''}</p></div><button disabled={disabled || busy || (!!editing && editing !== item.lineItemId)} onClick={() => open(item.lineItemId)} className="rounded border border-blue-300 px-3 py-1 text-sm text-blue-800 disabled:opacity-50">{item.issue ? 'Choose catalog item' : 'Change mapping'}</button></div>
      {editing === item.lineItemId && <div className="space-y-3 border-t pt-3">
        ...
        {choices && <>
          ...
          <button disabled={busy || !selectedItem} onClick={() => save()} className="rounded bg-blue-700 px-4 py-2 text-sm text-white disabled:opacity-50">{busy ? 'Saving…' : 'Save mapping & refresh review'}</button>
