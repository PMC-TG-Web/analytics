...
export default function DirectCostBillsPage() {
  ...
  const live = useRef({ busy, posting: posting || settingUp || mappingEditing, projectId, loadPreview });
  live.current = { busy, posting: posting || settingUp || mappingEditing, projectId, loadPreview };
  useEffect(() => {
  ...
  const expandedContent = <div className="space-y-5 p-4 sm:p-6">
    {error && <div role="alert" className="text-red-700"><p>{error}</p><button disabled={posting || busy} className="mt-2 rounded border px-3 py-2 text-sm" onClick={() => loadPreview(projectId, true)}>Refresh review</button></div>}
    {busy && <p role="status" className="p-8 text-center text-slate-500">Gathering synchronized daily logs and labor…</p>}
    {preview && <>
      {!mappingEditing && preview.lines.length > 0 && (!preview.review.connected || preview.lines.some(line => !preview.review.products[line.lineKey]) || preview.review.issues.some(issue => /mapping|offset category/i.test(issue))) && <ProjectSetup blockedReason={preview.issues.length ? 'Resolve the Cost Catalog or source issues below, then set up the missing QBO products here.' : undefined} key={`${preview.projectId}:${preview.month}`} companyId={companyId} projectId={preview.projectId} projectName={preview.projectName} month={preview.month} onBusy={setSettingUp} onComplete={async () => { await loadPreview(preview.projectId, true); setQueueRevision(n => n + 1); }} />}
      ...
      <CatalogMappingPanel key={`${preview.projectId}:${preview.month}`} companyId={companyId} projectId={preview.projectId} items={preview.catalogMappingItems} disabled={posting || settingUp} onBusy={setMappingEditing} onComplete={async () => { await loadPreview(preview.projectId, true); setQueueRevision(n => n + 1); }} />
  ...
  return <main className="mx-auto max-w-7xl space-y-6 p-4 text-slate-900 sm:p-8">
    ...
    <p className="mt-2 text-slate-600">Review monthly project costs for PMC Procore Direct Costs using current Cost Catalog prices. Concrete materials and the four specified pumping items under 03-300-40-30 are excluded; labor remains included.</p></div><span className="rounded-full bg-blue-50 px-3 py-1 text-sm font-medium text-blue-800">One bill per project / month</span></header>
    ...
    <div className="flex flex-wrap items-end gap-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <label className="flex flex-col gap-1">Month<input disabled={busy || posting || settingUp || mappingEditing} className="rounded border p-2" type="month" value={month} onChange={e => { setMonth(e.target.value); setProjectId(''); setPreview(null); setError(''); }} /></label>
    ...
    <ProjectBillQueue key={`${companyId}:${month}`} revision={queueRevision} companyId={companyId} month={month} disabled={busy || posting || settingUp || mappingEditing} selectedProjectId={projectId} onReview={loadPreview} onLoading={onQueueLoading} expandedContent={expandedContent} />
