...
export default function DirectCostBillsPage() {
  ...
  const live = useRef({ busy, posting: posting || settingUp || mappingEditing || reconciling, projectId, loadPreview });
  live.current = { busy, posting: posting || settingUp || mappingEditing || reconciling, projectId, loadPreview };
  useEffect(() => {
  ...
  const expandedContent = <div className="space-y-5 p-4 sm:p-6">
    {error && <div role="alert" className="text-red-700"><p>{error}</p><button disabled={posting || busy || reconciling} className="mt-2 rounded border px-3 py-2 text-sm" onClick={() => loadPreview(projectId, true)}>Refresh review</button></div>}
    {busy && <p role="status" className="p-8 text-center text-slate-500">Gathering synchronized daily logs and labor…</p>}
    {preview && <>
      {!reconciling && !mappingEditing && preview.lines.length > 0 && (!preview.review.connected || preview.lines.some(line => !preview.review.products[line.lineKey]) || preview.review.issues.some(issue => /mapping|offset category/i.test(issue))) && <ProjectSetup blockedReason={preview.issues.length ? 'Resolve the Cost Catalog or source issues below, then set up the missing QBO products here.' : undefined} key={`setup:${preview.projectId}:${preview.month}`} companyId={companyId} projectId={preview.projectId} projectName={preview.projectName} month={preview.month} onBusy={setSettingUp} onComplete={async () => { await loadPreview(preview.projectId, true); setQueueRevision(n => n + 1); }} />}
      ...
      {(preview.issues.length > 0 || preview.review.issues.length > 0) && <div role="alert"><p>Resolve these items before export:</p><ul className="list-disc pl-5">{[...new Set([...preview.issues, ...preview.review.issues])].map((issue, i) => <li key={i}><BillIssue message={issue} companyId={companyId} projectId={preview.projectId} sources={preview.issueSources} /></li>)}</ul></div>}
      <CatalogMappingPanel key={`catalog-mapping:${preview.projectId}:${preview.month}`} companyId={companyId} projectId={preview.projectId} items={preview.catalogMappingItems} disabled={posting || settingUp || reconciling} onBusy={setMappingEditing} onComplete={async () => { await loadPreview(preview.projectId, true); setQueueRevision(n => n + 1); }} />
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white"><div className="border-b p-5"><h3 className="font-semibold">Item details</h3><p className="text-sm text-slate-500">{preview.lines.length} aggregated items · Quantities replace the previous monthly totals.</p></div><div className="overflow-x-auto"><table className="w-full border-collapse text-left text-sm">
        ...
        <tbody>{preview.lines.map(line => <tr key={line.lineKey} className="hover:bg-slate-50"><td className="border-b p-3"><span className="font-medium">{line.description}</span><span className={`mt-1 block text-xs ${preview.review.products[line.lineKey] ? 'text-slate-500' : 'text-amber-700'}`}>{preview.review.products[line.lineKey] || `${line.costCode} · Product mapping needed`}</span></td><td className="border-b p-3 tabular-nums">{line.quantity}</td><td className="border-b p-3">{line.uom}</td><td className="border-b p-3 tabular-nums">${line.unitCost}{'rateSelection' in line && line.rateSelection !== 'category' && <span className="block text-xs text-gray-500">{line.rateSelection}</span>}</td><td className="border-b p-3 tabular-nums">{money(line.amount)}</td></tr>)}</tbody>
        <tfoot><tr><th className="p-3" colSpan={4}>Total</th><td className="p-3 font-semibold">{money(preview.total)}</td></tr></tfoot>
      </table></div></section>
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white"><div className="border-b p-5"><h3 className="font-semibold">Category details · Negative offsets</h3><p className="text-sm text-slate-500">Recalculated on each run. Customer / project stays blank on these lines.</p></div>{offsetsReady ? <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-slate-50"><tr>{['Category', 'Class', 'Customer / project', 'Amount'].map(h => <th key={h} className="p-3">{h}</th>)}</tr></thead><tbody>{([['material', materialTotal], ['labor', laborTotal]] as const).map(([kind, amount]) => <tr key={kind}><td className="p-3">{preview.review.offsets?.[kind]?.accountName || 'Mapping needed'}</td><td className="p-3">{preview.review.offsets?.[kind]?.className || 'Not assigned'}</td><td className="p-3 text-slate-500">—</td><td className="p-3 tabular-nums">{money(String(-Number(amount)))}</td></tr>)}</tbody></table></div> : <p className="p-5 text-sm text-amber-800">Offset mapping is unavailable on this server. Configure material and labor accounts and classes in the integration before posting.</p>}</section>
      {!preview.lines.length && <p>No eligible usage lines for this month.</p>}
      <details className="text-sm text-slate-600"><summary className="cursor-pointer font-medium">Source and exclusions</summary><p className="mt-2">{preview.sourceLogCount} productivity logs. Excluded: {preview.excluded.unapproved} unapproved, {preview.excluded.billingFile} billing-file, {preview.excluded.zeroUsage} zero-usage entries, {preview.excluded.concrete || 0} concrete material entries, {preview.excluded.pumpingEquipment || 0} excluded pumping-equipment entries, {preview.excluded.shopDrawings || 0} Shop Drawings vendor entries. Uses synchronized Procore data and current Cost Catalog rates. Catalog checked: {preview.catalogCheckedAt || 'Not available'}. Latest log update: {preview.latestSourceUpdate || 'None'}.</p></details>
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-slate-200 bg-slate-50 p-5"><div><p className="font-medium">{preview.issues.length || preview.review.issues.length ? 'Resolve issues before posting' : 'Save monthly bill'}</p><p className="mt-1 text-sm text-slate-600">{preview.review.canPost ? 'Save this reviewed monthly bill through the shared QBO service.' : preview.review.action === 'current' ? 'This monthly bill is up to date.' : 'Posting requires the shared QBO service and a valid current review.'}</p></div><div className="flex flex-wrap gap-3"><button className="rounded-lg border border-slate-300 px-5 py-2 font-medium disabled:opacity-50" disabled={posting || mappingEditing || settingUp || reconciling || preview.issues.length > 0 || !preview.lines.length || preview.review.action === 'reconcile'} onClick={download}>Export draft</button><button disabled={posting || mappingEditing || settingUp || reconciling || !preview.review.canPost || preview.issues.length > 0 || preview.review.issues.length > 0} onClick={saveBill} className="rounded-lg bg-blue-700 px-5 py-2 font-medium text-white disabled:opacity-50">{posting ? 'Saving to QBO…' : preview.review.billId ? 'Update bill in QBO' : 'Create bill in QBO'}</button></div></div>
    </>}
  </div>;
  return <main className="mx-auto max-w-7xl space-y-6 p-4 text-slate-900 sm:p-8">
    ...
    <div className="flex flex-wrap items-end gap-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <label className="flex flex-col gap-1">Month<input disabled={busy || posting || settingUp || mappingEditing || reconciling} className="rounded border p-2" type="month" value={month} onChange={e => { setMonth(e.target.value); setProjectId(''); setPreview(null); setError(''); }} /></label>
    ...
    <ProjectBillQueue key={`${companyId}:${month}`} revision={queueRevision} companyId={companyId} month={month} disabled={busy || posting || settingUp || mappingEditing || reconciling} selectedProjectId={projectId} onReview={loadPreview} onLoading={onQueueLoading} expandedContent={expandedContent} />
