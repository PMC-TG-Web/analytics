...
export default function DirectCostBillsPage() {
  ...
  const live = useRef({ busy, posting: posting || settingUp, projectId, loadPreview });
  live.current = { busy, posting: posting || settingUp, projectId, loadPreview };
  useEffect(() => {
    ...
    async function check() {
      ...
      try {
        ...
        setSyncMessage('Checking Procore PO prices…');
        ...
        setSyncMessage(result.status === 'waiting' ? 'Procore sync is busy; automatic checks will resume.' : `Automatic PO checks active · Last check ${new Date().toLocaleTimeString()}`);
        if (!live.current.busy && !live.current.posting) {
          ...
          if (result.status === 'synced' && live.current.projectId === result.projectId) await live.current.loadPreview(result.projectId, true);
        }
  ...
  return <main className="mx-auto max-w-7xl space-y-6 p-4 text-slate-900 sm:p-8">
    ...
    <p className="mt-2 text-slate-600">Review monthly project costs for PMC Procore Direct Costs. Concrete material usage is excluded; concrete labor remains included.</p></div><span className="rounded-full bg-blue-50 px-3 py-1 text-sm font-medium text-blue-800">One bill per project / month</span></header>
    ...
    <div className="flex flex-wrap items-end gap-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <label className="flex flex-col gap-1">Month<input disabled={busy || posting || settingUp} className="rounded border p-2" type="month" value={month} onChange={e => { setMonth(e.target.value); setProjectId(''); setPreview(null); setError(''); }} /></label>
    ...
    <ProjectBillQueue key={`${companyId}:${month}`} revision={queueRevision} companyId={companyId} month={month} disabled={busy || posting || settingUp} selectedProjectId={projectId} onReview={loadPreview} onLoading={onQueueLoading} expandedContent={expandedContent} />
