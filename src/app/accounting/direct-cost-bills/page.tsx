...
export default function DirectCostBillsPage() {
  ...
  const displayLines = billDisplayLines(preview?.lines || [], preview?.review.itemGroups);
  const laborTotal = (displayLines.filter(l => l.sourceType === 'timecard' || /^(labor|l)$/i.test(l.costType || '')).reduce((sum, l) => sum + Math.round(Number(l.amount) * 100), 0) / 100).toFixed(2);
  const materialTotal = (displayLines.filter(l => (/^(materials|m)$/i.test(l.costType || '') || (/^(other|o|equipment|e|commitments|c)$/i.test(l.costType || '') && preview?.review.offsetCategories?.[l.lineKey] === 'material'))).reduce((sum, l) => sum + Math.round(Number(l.amount) * 100), 0) / 100).toFixed(2);
  ...
  const expandedContent = <div className="space-y-5 p-4 sm:p-6">
    ...
    {preview && <>
      <ProjectLineRules key={`line-rules:${preview.projectId}:${preview.month}`} companyId={companyId} projectId={preview.projectId} month={preview.month} items={preview.ruleItems || []} disabled={posting || settingUp || reconciling || foodEditing || mappingEditing} onBusy={setRuleEditing} onComplete={async () => { await loadPreview(preview.projectId, true); setQueueRevision(n => n + 1); }} />
      <FoodTotalInput key={`food:${preview.projectId}:${preview.month}`} companyId={companyId} projectId={preview.projectId} month={preview.month} saved={preview.food.saved} entries={preview.food.entries} logCount={preview.food.logCount} disabled={posting || settingUp || mappingEditing || ruleEditing || reconciling} onBusy={setFoodEditing} onComplete={async () => { await loadPreview(preview.projectId, true); setQueueRevision(n => n + 1); }} />
      {!foodEditing && !reconciling && !mappingEditing && !ruleEditing && preview.lines.length > 0 && (!preview.review.connected || preview.lines.some(line => !preview.review.products[line.lineKey]) || preview.review.issues.some(issue => /mapping|offset category/i.test(issue))) && <ProjectSetup blockedReason={preview.issues.length ? 'QBO product setup is paused until the listed items are resolved.' : undefined} key={`setup:${preview.projectId}:${preview.month}`} companyId={companyId} projectId={preview.projectId} projectName={preview.projectName} month={preview.month} onBusy={setSettingUp} onComplete={async () => { await loadPreview(preview.projectId, true); setQueueRevision(n => n + 1); }} />}
      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white"><div className="border-b p-5"><h3 className="font-semibold">Item details</h3><p className="text-sm text-slate-500">{displayLines.length} aggregated items · Matching items across POs and daily logs are combined. Quantities replace the previous monthly totals. Existing screeder prices are kept from QBO; lines added in QBO are retained.</p></div><div className="overflow-x-auto"><table className="w-full border-collapse text-left text-sm">
        ...
        <tbody>{displayLines.map(line => <tr key={line.lineKey} className="hover:bg-slate-50"><td className="border-b p-3"><span className="font-medium">{line.description}</span><span className={`mt-1 block text-xs ${preview.review.products[line.lineKey] ? 'text-slate-500' : 'text-amber-700'}`}>{preview.review.products[line.lineKey] || `${line.costCode} · QBO product setup needed`}</span>{preview.review.itemClasses?.[line.lineKey] && <span className="mt-1 block text-xs text-slate-500">Class: {preview.review.itemClasses[line.lineKey]}</span>}{line.sourceKeys.length > 1 && <details className="mt-2 text-xs text-slate-600"><summary className="cursor-pointer">{line.sourceKeys.length} combined source lines</summary><ul className="mt-1 space-y-1">{line.sourceKeys.map(key => { const source = preview.lines.find(row => row.lineKey === key)!; const po = 'purchaseOrder' in source ? source.purchaseOrder : null; return <li key={key}>{po?.number || source.description}{po?.title ? ` — ${po.title}` : ''}: {source.quantity} {source.uom}; {Array.from(new Set(source.sourceLogs.map(log => log.date))).sort().join(', ')}</li>; })}</ul></details>}</td><td className="border-b p-3 tabular-nums">{line.quantity}</td><td className="border-b p-3">{line.uom}</td><td className="border-b p-3 tabular-nums">${line.unitCost}{preview.review.qboPrices?.[line.lineKey] && <span className="block text-xs text-slate-500">QBO price</span>}{line.sourceType === 'manual_food' && <span className="block text-xs text-gray-500">Entered monthly total</span>}{'rateSelection' in line && line.rateSelection !== 'category' && <span className="block text-xs text-gray-500">{line.rateSelection}</span>}</td><td className="border-b p-3 tabular-nums">{money(line.amount)}</td></tr>)}</tbody>
