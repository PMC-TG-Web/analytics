...
export async function POST(request: Request) {
  try {
    ...
    for (const host of hostCandidates.candidates) {
      ...
      return NextResponse.json({
        success: true,
        source: "estimating.proposal_show",
        companyId,
        projectId,
        proposalId,
        baseUrl: host,
        ...
        proposal,
        raw: payload,
        embeddedLineItemGroups: extractEmbeddedArray(proposal, ["line_item_groups", "groups"]),
        embeddedLineItems: extractEmbeddedArray(proposal, ["line_items", "items"]),
      });
    }
