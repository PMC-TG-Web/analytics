...
export async function POST(request: Request) {
  return withProcoreLiveApiBypassForSyncSecret(request, async () => {
    ...
    const userAccessToken =
      cookieStore.get("procore_access_token")?.value || String(body.accessToken || "").trim() || "";
    ...
    if (userAccessToken) {
      ...
      try {
        ...
        return NextResponse.json(
          { error: "Missing access token. Please authenticate via OAuth first or provide accessToken.", connectUrl: "/api/auth/procore/login" },
          { status: 401 }
        );
      }
    }
