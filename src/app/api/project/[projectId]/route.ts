...
export async function GET(request: NextRequest, props: { params: Promise<{ projectId: string }> }) {
  try {
    ...
    const [projectDetails, timecardEntries, budgetData, changeOrderData] = await Promise.allSettled([
      // Use v1.1 API direct endpoint for project detail lookup
      (async () => {
        console.log(`[Project Overview] Fetching v1.1 project ${projectId}`);
        const endpoint = `/rest/v1.1/projects/${projectId}`;
        console.log(`[Project Overview] Endpoint: ${endpoint}`);
        const response = await makeRequest(endpoint, accessToken);
        console.log(`[Project Overview] Response type: ${typeof response}, has name: ${response?.name ? 'yes' : 'no'}`);
        if (response && response.id) {
          console.log(`[Project Overview] Found project: ${response.name}`);
          return response;
        }
