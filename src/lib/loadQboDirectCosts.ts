...
export async function loadQboDirectCosts(companyId: string, projectId: string, month: string) {
  ...
  const [logs, items, aliases, timecards, laborRates] = await Promise.all([
    ...
    prisma.purchaseOrderLineItemContractDetail.findMany({ where: { procoreCompanyId: companyId, procoreProjectId: projectId },
      select: { procoreId: true, description: true, unitCost: true, uom: true, updatedAt: true, costCode: true, costType: true, procorePurchaseOrderContractId: true, purchaseOrderContract: { select: { number: true, title: true } } } }),
    ...
    loadQboDirectCostLaborRates(companyId, projectId),
  ]);
  const summary = aggregateDirectCosts(logs.map(log => ({ ...log, id: log.procoreId || log.id })), items, new Map(aliases.map(a => [a.source_line_item_id, a.target_line_item_id])));
  ...
  return {
    schemaVersion: 2, scope: 'productivity_and_timecards', generatedAt: new Date().toISOString(),
    ...
    notes: ['Uses approved productivity logs and non-deleted timecards, including timecards whose source has no approval status.', 'Labor uses configured hourly costs from the selected primary/base estimate; unknown or conflicting rates block posting.', 'Negative category offsets are not included.'],
  };
}
