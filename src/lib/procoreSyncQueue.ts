  ...
  return prisma.$executeRawUnsafe(
    `
      ...
      DO UPDATE SET
        project_number = COALESCE(EXCLUDED.project_number, procore_sync_project_states.project_number),
        project_name = COALESCE(EXCLUDED.project_name, procore_sync_project_states.project_name),
        -- Queue a changed estimate immediately while preserving the age of
        -- work that is already overdue. Resetting every edit to the Unix epoch
        -- lets repeatedly changing projects starve the rest of the queue.
