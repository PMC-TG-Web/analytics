...
function render(opened, changed, needsReconciliation = false) {
 const states = [opened, false, { manuallyChanged: changed, billNumber: 'Example 001', current: { rows: [], total: 0, note: '' }, proposed: { rows: [], total: 0, note: 'New catalog note' } }, '', false, true];
...
test('reconciliation offers keeping QBO additions while refreshing Procore lines', () => { const html = render(true, true); assert.match(html, /Keep lines added directly in QBO/); assert.match(html, /on this and future updates/); assert.match(html, /Procore-generated lines still update normally/); });
