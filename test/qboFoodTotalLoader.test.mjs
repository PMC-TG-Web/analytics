...
async function load(saved, rules = [], timecards = []) {
 ...
 const imports={ './qboDirectCostExclusions.js':exclusions, './qboBillLineRules':{projectPrice:()=>null}, './prisma':{prisma}, './qboDirectCosts':costs,'@prisma/client':{Prisma}, './qboDirectCostLabor':labor, './loadQboDirectCostLaborRates':{loadQboDirectCostLaborRates:async()=>({rates:[{costCode:'03-300-20-10',rate:'50',lineItemId:'5',updatedAt:date.toISOString()}]})},'./loadQboCostCatalog':{loadQboCostCatalog:async()=>({fetchedAt:date.toISOString(),items:[]})},'./qboCostCatalog':{catalogSnapshotIssue:()=>null},'./qboCatalogMapping':{purchasePriceFallback:()=>null,catalogSourceSignature:()=> 'a'.repeat(64),duplicateCatalogSourceIds:()=>new Set(),mappedCatalogPrice:source=>{assert.notEqual(source.description,'Food');return {unitCost:3.5,evidence:null,issue:null};}},'./qboDirectCostCoding':coding,'./qboFoodTotal':food,'./estimatingCostCodeCrosswalk':{loadEstimatingCostCodeCatalog:()=>new Map()}};
